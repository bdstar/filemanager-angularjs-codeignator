...
angular.module('angular-growl').directive('growl', [function () {
	...
	return {
		restrict: 'A',
		template: '<div class="growl-container" x-ng-class="wrapperClasses()">' + '<div class="growl-item" x-ng-repeat="message in growlMessages.directives[referenceId].messages" x-ng-class="alertClasses(message)" x-ng-click="stopTimeoutClose(message)">' + '<button type="button" class="growl-btn" x-ng-click="growlMessages.deleteMessage(message)" x-ng-show="!message.disableCloseButton">&times;</button>' + '<button type="button" class="growl-btn" x-ng-show="showCountDown(message)">{{message.countdown}}</button>' + '<h4 class="growl-title" x-ng-show="message.title" x-ng-bind="message.title"></h4>' + '<div class="growl-message" x-ng-bind-html="message.text"></div>' + '</div>' + '</div>',
		replace: false,
		scope: {
			reference: '@',
			inline: '=',
		...
		controller: [
			...
			function ($scope, $interval, growl, growlMessages) {
				...
				$scope.alertClasses = function (message) {
					return {
						'growl-alert-success': message.severity === 'success',
						'growl-alert-error': message.severity === 'error',
						'growl-alert-danger': message.severity === 'error',
						'growl-alert-info': message.severity === 'info',
						'growl-alert-warning': message.severity === 'warning',
						'growl-icon': message.disableIcons === false,
						'growl-alert-dismissable': !message.disableCloseButton
					};
				};
